...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const orderId =
      body.orderId || Math.floor(Math.random() * 900000 + 100000);
    ...
    // Get Hostinger SMTP environment variables
    let SMTP_HOST = Deno.env.get("SMTP_HOST");
    const SMTP_PORT = 587; // FIXED: use 587 always
    const SMTP_USER = Deno.env.get("SMTP_USER");
    const SMTP_PASS = Deno.env.get("SMTP_PASS");

    console.log("🔑 SMTP configured:", SMTP_HOST);

    // Fallback if wrong host saved
    if (!SMTP_HOST || SMTP_HOST === "") {
      SMTP_HOST = "smtp.hostinger.com";
    }

    // Create SMTP client (Hostinger compatible)
    const client = new SMTPClient({
      connection: {
        hostname: SMTP_HOST,
        port: SMTP_PORT,
        tls: false, // Hostinger requires startTLS on 587
        starttls: true,
        auth: {
          username: SMTP_USER,
          password: SMTP_PASS,
