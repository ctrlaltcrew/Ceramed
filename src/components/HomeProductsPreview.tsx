...
const HomeProductsPreview = () => {
  ...
  const fetchProducts = async () => {
    try {
      ...
      // Add fallback images for products without images
      const processed =
        data?.map((p, i) => ({
          ...p,
          image_url:
  ...
  return (
    <section
      ...
      className="py-20 bg-gradient-to-b from-white to-accent/10 overflow-hidden"
    >
      <div className="container mx-auto px-6">
        ...
        {/* Continuous Sliding Products */}
        <div className="relative w-full overflow-hidden">
          <div
            className="flex gap-6 animate-scroll-continuous"
            style={{ width: "max-content" }}
          >
            {[...products, ...products, ...products].map((product, index) => (
              <div
                key={index}
                className="min-w-[250px] bg-white shadow-md rounded-2xl p-5 flex-shrink-0 hover:shadow-xl transition-all duration-300"
              >
        ...
        <div className="text-center mt-10">
          ...
          >
            <Button className="group bg-secondary hover:bg-secondary/90 text-white">
              View More Products
